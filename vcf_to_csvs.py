...
def write_filtered_vcf(filtered_vcf_records,output_file):
    with open(output_file, 'w') as f:
        for record in filtered_vcf_records:
            f.write(record)
bed_positions = read_bed_file(r'script_resources/Covered_regions.bed')

    ########################################### IMPORTING THE VCF DATA AND EXPANDING THE DEPTH COLUMNS ##########################
def process(filepath,semi_output_dir,output_dir):
        ...
        df_1 = pd.read_excel(r'script_resources/consequence.xlsx')

        merged_1 = pd.merge(vcf_final, df_1, on='consequence', how='left', sort=False)

        df_2 = pd.read_excel(r'script_resources/IMPACT.xlsx')
        ...
        df_gene = pd.read_excel(r'script_resources/Conditions_final_genes.xlsx')
        ...
        df_3 = pd.read_excel(r'script_resources/new_final_output_concatenated.xlsx')
        ...
        merged_3.to_csv(f"{semi_output_dir}/{filename}.csv",index=False)
        ...
        consequence_filter['DP'] = consequence_filter['DP'].replace('nan','0').astype('int64')
        ...
        drop_duplicates_filter = drop_duplicates_filter.set_index(['Gene Name', 'Gene', 'Gene_Score', 'rsID', 'Literature', 'CHROM', 'POS', 'REF',
            ...
            'IMPACT_score', 'ClinVar_CLNDN', 'ClinVar_CLNSIG', 'ClinVar_CLNREVSTAT',
        ...
        return f"{semi_output_dir}/{filename}.csv",f"{output_dir}/{filename}.csv"
